...
#
#This is the python script that I wrote to automate my house.
#Official Code Initiatiation (OCI Date): 12/25/2016
#Official Code Termination (OCT Date): 8/3/2017, better version created
...
def getmail():
    ...
    conn.login("insertemailhere", "insertpasswordhere")
    conn.select()
...
def genresponse(flags):
    ...
    if flags[1] == 1:
        ...
        if flags[6] == 1 and flags[17] == 1 and flags[2] == 1 and flags[16]:
            genmes = genmes + "Aidan's room."
        if flags[7] == 1 and flags[18] == 1 and flags[2] == 1 and flags[16]:
            genmes = genmes + "Brendan's room."
        if flags[8] == 1 and flags[2] == 1 and flags[16]:
            genmes = genmes + "the kitchen."
        if flags[9] == 1 and flags[2] == 1 and flags[16]:
            genmes = genmes + "the living room."
        if flags[10] == 1 and flags[2] == 1 and flags[16]:
            genmes = genmes + "the TV room."
        if flags[11] == 1 and flags[2] == 1 and flags[16]:
            genmes = genmes + "the master bedroom."
        if flags[12] == 1 and flags[2] == 1 and flags[16]:
            genmes = genmes + "the dining room."
        if flags[15] == 1 and flags[2] == 1 and flags[16]:
            if sender == "Michael":
